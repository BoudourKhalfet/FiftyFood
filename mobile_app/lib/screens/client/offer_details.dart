...
class _OfferDetailsPageState extends State<OfferDetails> {
  ...
  Future<void> _createOrderAndPay(
    ...
    try {
      ...
      // Handle payment based on method - NO ORDER CREATED YET
      ...
      if (paymentMethod == 'CARD') {
        ...
        // Stripe payment - create order first with PENDING status

        try {

          debugPrint('Creating order first with PENDING status for Stripe');

          

          // Create order with PENDING status first

          final tempOrderDetails = Map<String, dynamic>.from(orderDetails);

          tempOrderDetails.remove('status');
          ...
          final response = await http.post(
            ...
            body: jsonEncode(tempOrderDetails),
          ...
          final orderId = orderData['order']?['id'] ?? orderData['id'];

          

          debugPrint('Order created with PENDING status: $orderId');
          ...
          final intentData = await PaymentService.createStripeIntent(
            ...
            items: {
              ...
              'price': widget.offer['discountedPrice'],
            ...
            deliveryPhone: orderDetails['deliveryPhone'],
          ...
          // Navigate to payment screen with real order ID

          final checkoutScreen = OrderCheckoutScreen(

            orderId: orderId, // Real order ID

            totalAmount: totalAmount,

            orderDetails: orderDetails,

            initialMethod: AppPaymentMethod.values.firstWhere((method) => method.name == 'card'),

            clientSecret: intentData['clientSecret'],

            createOrderAfterPayment: false, // Order already created
        ...
        // PayPal payment - create order first with PENDING status

        try {

          debugPrint('Creating order first with PENDING status for PayPal');
          ...
          // Determine appropriate return URLs based on platform

          final String returnUrl, cancelUrl;

          if (kIsWeb) {

            // Web platform - use localhost URLs

            returnUrl = 'http://localhost:52531/payment/paypal/return';

            cancelUrl = 'http://localhost:52531/payment/paypal/cancel';

          } else {

            // Mobile platform - use deep links

            returnUrl = 'fiftyfood://payment/paypal/return';

            cancelUrl = 'fiftyfood://payment/paypal/cancel';

          }



          debugPrint('PayPal: About to call PaymentService.createPayPalPayment');

          debugPrint('PayPal: Order ID: $orderId');

          debugPrint('PayPal: Amount: $totalAmount');

          

          final paypalData = await PaymentService.createPayPalPayment(

            orderId: orderId, // Use real order ID now
          ...
          debugPrint('PayPal: Payment created successfully: ${paypalData.toString()}');
          ...
          Navigator.of(context).pushReplacement(
            ...
            MaterialPageRoute(
              ...
              builder: (context) => OrderCheckoutScreen(
                ...
                initialMethod: AppPaymentMethod.values.firstWhere((method) => method.name == 'paypal'),
