    ...
    def _visibility_content_cf(
        ...
        if vis_share >= eng_share and vis_share > 0:
            ...
            _vis_msg = (
                ...
                "D\u00e9taillez davantage le contenu pour rassurer les clients \u2014 "
            ...
            _vis_msg = (
                ...
                "Un d\u00e9tail concret dans la description (portions, ingr\u00e9dients, poids) "
                "pourrait inverser la tendance."
            )
        else:
            _vis_msg = (
                "Votre offre manque d\u2019attractivit\u00e9. "
                "Enrichissez la description "
...
if __name__ == "__main__":
    import sys
    from optimizer import _FEAT_FR
    ...
    for _, row in results.iterrows():
        ...
        _color = {"HIGH": "🔴", "MEDIUM": "🟡", "LOW": "🟢"}.get(level, "⚪")
        risk_pct = int(round(score * 100))
        print(f"\n{thin}")
        print(f"  {_color} {oid}  │  {label}")
        print(f"     Risque: {risk_pct}% ({level})   Confiance: {conf}")

        # ── Section 1: DIAGNOSTIC (why is this offer at risk?) ────────────
        if diag_out and diag_out.get("summary"):
            print(f"\n     📊 DIAGNOSTIC")
            print(f"     {diag_out['summary']}")
            axes = diag_out.get("axes", [])
            if axes:
                bar_width = 30
                for a in axes:
                    pct = int(round(a["share"] * 100))
                    filled = int(round(a["share"] * bar_width))
                    bar = "█" * filled + "░" * (bar_width - filled)
                    print(f"        {bar} {pct:>3}%  {a['label']}")

        # ── Section 2: ACTION PLAN (what should the restaurant DO) ────────
        ap = row.get("action_plan")
        if ap:
            curr_pct = int(round(ap["current_risk"] * 100))
            opt_pct  = int(round(ap["optimized_risk"] * 100))
            red_pts  = int(round(ap["total_reduction"] * 100))

            print(f"\n     ✅ PLAN D'ACTION RECOMMANDÉ")
            print(f"     {ap['message']}")
            print(f"     Risque: {curr_pct}% ──→ {opt_pct}%  (−{red_pts} pts)")

            if ap.get("changes"):
                print(f"     Modifications:")
                for ch in ap["changes"]:
                    feat = ch["feature"]
                    curr_v = ch["current"]
                    sugg_v = ch["suggested"]
                    scope = ch.get("scope", "now")
                    scope_tag = "🔧 maintenant" if scope == "now" else "📅 prochaine fois"
                    if feat == "discount_rate":
                        print(f"        • Remise: {int(round(curr_v))}% → {int(round(sugg_v))}%  [{scope_tag}]")
                    elif feat == "description_length":
                        print(f"        • Description: {int(round(curr_v))} → {int(round(sugg_v))}+ car.  [{scope_tag}]")
                    elif feat == "time_to_pickup_hours":
                        print(f"        • Délai publication: {round(curr_v, 1)}h → {round(sugg_v, 1)}h  [{scope_tag}]")
                    else:
                        print(f"        • {feat}: {curr_v} → {sugg_v}  [{scope_tag}]")

            # Alternatives
            for alt in ap.get("alternatives", []):
                alt_red = alt.get("reduction_pts", "?")
                alt_risk = int(round(alt["optimized_risk"] * 100))
                # Fix duplicated label: strip the label prefix from the message if present
                alt_msg = alt["message"]
                alt_label = alt["label"]
                for prefix in ["Option legere: ", "Option maximale: "]:
                    if alt_msg.startswith(prefix):
                        alt_msg = alt_msg[len(prefix):]
                        break
                print(f"\n     💡 {alt_label} (risque → {alt_risk}%, −{alt_red} pts)")
                print(f"        {alt_msg}")
            ...
            if exploited:
                top_syn = exploited[0]
                if top_syn["synergy"] >= 0.05:
                    f1 = _FEAT_FR.get(top_syn["features"][0], top_syn["features"][0])
                    f2 = _FEAT_FR.get(top_syn["features"][1], top_syn["features"][1])
                    print(f"     🔗 Synergie détectée: {f1} × {f2} — agir sur les deux amplifie l'effet")

        # ── Section 3: DÉTAIL DES FACTEURS (supporting explanations) ──────
        factors = row.get("top_factors", [])
        actionable_factors = [f for f in factors if f.get("counterfactual")]
        context_factors    = [f for f in factors if not f.get("counterfactual")]

        if actionable_factors:
            print(f"\n     📋 DÉTAIL DES FACTEURS ACTIONNABLES")
            for f in actionable_factors:
                cf = f["counterfactual"]
                shap = f["shap_contribution"]
                impact = "▲▲▲" if shap >= 1.0 else "▲▲" if shap >= 0.3 else "▲"
                print(f"        {impact} {f['feature']} = {f['value']}  (impact SHAP: +{shap:.3f})")
                msg = cf.get("message", "")
                if msg:
                    print(f"           → {msg}")

        if context_factors:
            print(f"\n     ℹ️  FACTEURS CONTEXTUELS (non modifiables sur cette offre)")
            for f in context_factors:
                shap = f["shap_contribution"]
                if shap >= 0.01:
                    impact = "▲▲▲" if shap >= 1.0 else "▲▲" if shap >= 0.3 else "▲"
                    print(f"        {impact} {f['feature']} = {f['value']}  (impact: +{shap:.3f})")

        # If no action plan exists, show a note
        if not ap:
            actionable_tips = [f for f in factors if f.get("counterfactual") and f["counterfactual"].get("message")]
            if actionable_tips:
                print(f"\n     💡 CONSEILS INDIVIDUELS")
                for f in actionable_tips:
                    print(f"        → {f['counterfactual']['message']}")
            elif level == "LOW":
                print(f"\n     ✅ Offre bien configurée — aucune action requise.")
            else:
                print(f"\n     ⚠️  Aucun plan d'action généré — les leviers disponibles ont un impact limité.")
