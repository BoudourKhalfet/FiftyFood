...
export async function trainAndForecast(
  ...
  const avg =
    train.reduce((sum, d) => sum + d.revenue, 0) / Math.max(train.length, 1);

  const recent = train.slice(-7);
  const older = train.slice(-14, -7);
  const recentAvg =
    recent.reduce((s, d) => s + d.revenue, 0) / Math.max(recent.length, 1);
  const olderAvg = older.length
    ? older.reduce((s, d) => s + d.revenue, 0) / older.length
    : recentAvg;
  const trend = Math.max(
    -0.2,
    Math.min(0.2, (recentAvg - olderAvg) / Math.max(olderAvg, 1)),
  );

  const lastDate = new Date(
    daily[daily.length - 1]?.date ?? new Date().toISOString().slice(0, 10),
  );
  ...
  for (let i = 1; i <= horizon; i += 1) {
    ...
    const predictedRevenue = Math.round(
      avg * (1 + trend * (i / horizon)) * weekendBoost,
    );
