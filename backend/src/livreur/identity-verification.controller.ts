...
import { VerifyCINDto, VerifyFaceDto, LivenessVerifyDto, IdentityVerificationResponseDto } from './dto/identity-verification.dto';
...
export class IdentityVerificationController {
  ...
  async verifyCIN(
    ...
    try {
      ...
      if (dto.cinBackImageBase64) {
        const tunisianVerification = await this.identityVerificationService.verifyTunisianDocument(
          dto.cinFrontImageBase64,
          dto.cinBackImageBase64,
          dto.cinNumber,
        );

        this.logger.log(`[CIN] Doc check: valid=${tunisianVerification.isValid} score=${tunisianVerification.score.toFixed(2)} barcode=${tunisianVerification.checks.hasValidBarcode} flag=${tunisianVerification.checks.hasTunisianFlag} cinMatch=${tunisianVerification.checks.userCinMatch}`);
        
        if (!tunisianVerification.isValid) {
          await this.identityVerificationService.storeCINVerificationStatus(userId, 'FAILED');
        ...
        frontExtractedCIN = tunisianVerification.checks.ocrCin || frontExtractedCIN;
      }
      ...
      const baseUrl = process.env.PUBLIC_BACKEND_URL || process.env.BASE_URL || 'http://localhost:3000';
  ...
  async verifyFace(
    ...
    if (!dto.livenessSessionId) {
      throw new BadRequestException('Liveness verification must be completed first.');
    }
    if (!this.livenessSessionService.isSessionConsumed(dto.livenessSessionId, userId)) {
      this.logger.warn(`[FaceVerify] User ${userId} provided invalid/unconsumed session: ${dto.livenessSessionId}`);
      throw new BadRequestException('Liveness verification must be completed first. Please complete the liveness challenge.');
    }
  ...
  async issueLivenessSession(@Req() req: ReqWithUser) {
    ...
    this.logger.log(`[Liveness] Session issued for user ${userId}: ${session.sessionId}`);
    return session;
  }
  ...
  async verifyLiveness(
    ...
    const userId = req.user.sub;
    this.logger.log(`[Liveness] Challenges ${JSON.stringify(dto.challenges)} from user ${userId} with ${dto.frames.length} frames, ${dto.telemetry?.length || 0} telemetry`);
    ...
    try {
      ...
      const sessionCheck = this.livenessSessionService.validateSession(
        dto.sessionId, dto.nonce, userId,
      );
      if (!sessionCheck.valid) {
        return {
          success: false, isLive: false, confidence: 0,
          reason: sessionCheck.error, riskLevel: 'HIGH',
        };
      }

      // Use server-authoritative challenges (ignore client-sent challenges)
      const serverChallenges = this.livenessSessionService.getSessionChallenges(dto.sessionId);
      if (!serverChallenges) {
      ...
      const payloadHash = createHash('sha256')
        .update(dto.sessionId + dto.nonce + dto.frames.length + (dto.telemetry?.length || 0))
        .digest('hex');

      if (this.livenessSessionService.isReplay(payloadHash)) {
        return {
          success: false, isLive: false, confidence: 0,
          reason: 'Replay attack detected', riskLevel: 'HIGH',
        };
      }

      // 1. Call Python liveness service with frames + telemetry (server validates everything)
      const livenessResult = await this.identityVerificationService.performLivenessVerification(
        dto.frames,
      ...
      if (cinImage && dto.selfieImageBase64) {
        ...
        this.logger.log(`[Liveness] Sampling frames ${sampleIndices.join(', ')} for face comparison`);
        ...
        const avgScore = faceResults.reduce((sum, r) => sum + r.matchScore, 0) / faceResults.length;
        ...
        this.logger.log(`[Liveness] Face comparison: ${matchCount}/3 matches, avg_score=${avgScore.toFixed(3)}, final_match=${faceMatch.isMatch}`);
      }
      ...
      throw new BadRequestException('Liveness verification failed. Please try again.');
    }
  }
